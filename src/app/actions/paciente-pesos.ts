...
export async function atualizarPesoPaciente(
  _estadoAnterior: EstadoPeso,
  formData: FormData,
): Promise<EstadoPeso> {
  await exigirPermissao(
  ...
  if (
    ...
    return {
      ok: false,
      mensagem:
        "Dados inválidos para editar a pesagem.",
    };
  }
  ...
  if (!paciente.ativo) {
    return {
      ok: false,
      mensagem:
        "Não é possível editar peso de um paciente inativo.",
    };
  }
  ...
  // Recalcula o peso atual a partir da pesagem ativa mais recente.
  // Assim, editar uma pesagem antiga não a transforma no peso atual.
